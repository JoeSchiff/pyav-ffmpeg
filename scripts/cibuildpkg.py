...
def run(cmd, env=None):
    ...
    try:
        subprocess.run(cmd, check=True, env=env, shell=True, stderr=subprocess.PIPE, text=True)
...
class Builder:
    ...
    def _build_with_autoconf(self, package: Package, for_builder: bool) -> None:
        ...
        if package.name == "ffmpeg":
            print(99999999)
            try:
                shutil.copy("C:\\cibw\\vendor\\lib\\pkgconfig\\aom.pc", "C:\\msys64\\usr\\share\\aom.pc")
